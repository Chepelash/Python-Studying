...
def extract_names(filename):
    ...
    y = soup.table.find_all('b')
    year_lst = [re.findall(r'>([\d]+?)<', str(item))[0] for item in y[3::2]]
    babies = {}
    for item in tr[2:]:
        lst = re.findall(r'>\s*?(\s.+?\s)\s*?<', str(item))
        for i in range(len(lst)):
            lst[i] = lst[i].strip()
        babies.update({lst[1]: lst[2:]})
    return babies
...
def print_names(babynames):
    ...
    while True:
        print("года: ", ', '.join(year_lst))
        year = input('По какому году вывести статистику? ')
        if year == 'q':
            sys.exit(0)
        elif year not in year_lst:
            print('Статистики по этому году нет.')
            continue
        else:
            column = year_lst.index(year)
